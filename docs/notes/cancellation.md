...
- **优雅退出有上限。** 收到 SIGTERM 后，`http.Server.Shutdown` 会等进行中的请求处理完（包括结算），但最多等 30 秒；更长的流会在那之后随进程退出被强行中断，而且没有结算。M2 接入计费时要补上：超时后先取消剩余的请求，让它们按中途断开的规则结算，再退出。
...
- 退出时如果不想干等长流，而是主动取消所有进行中的请求、按估算结算后再退出，可以给 `http.Server` 设置 `BaseContext`，退出时取消它。
